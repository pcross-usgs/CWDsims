...
|           | Parameter         | Symbol    | Code name |Comments|
|----------------- | ----------------------- |----------   |---------------- | ----------------------------------------------|
...
| |  | |  | |
|Vital rates       | natural survival | $\delta_{i,a}$ |fawn.an.sur, juv.an.sur, ad.an.f.sur, ad.an.m.sur | entered as annual proportions and later converted to monthly |
|                | reproduction      | $\alpha_{a>1}$|fawn.repro, juv.repro, ad.repro| may be greater than one to account for twins | 
|                | hunting mortality | $\kappa_{i,a}$ |hunt.mort.fawn, hunt.mort.juv.f, hunt.mort.juv.m, hunt.mort.ad.f, hunt.mort.ad.m|  |
|                | disease mortality | $\rho$ | p | dictates the rate of movement among infectious sub-categories |
| | relative risk | $RR$ | rel.risk | increase in hunting mortality associated with being CWD positive |
| |  | |  | |
|Transmission    | envi. transmission| $\epsilon$ | envi.foi | annual probability from 0 to 1 |
|                | transmission coefficient | $\beta_{i}$ | beta.f, beta.m | direct transmission |
|                | male scaling      | $\gamma$ | | male transmission = $\beta_m = \beta_f \gamma$|
|                | mixing parameter  | $\theta$ | theta | $\theta = 1$ is frequency dependent transmission, $\theta = 0$ is density dependent transmission |
| |  | |  | |
|Stochasticity   | vital rate variation  | $\sigma^2$ |repro.var, fawn.sur.var, sur.var, hunt.var| variance for the survival, hunting and reproduction rates |
| |  | |  | |
| Intitial conditions | prevalence | | ini.fawn.prev, ini.juv.prev, ini.ad.f.prev, ini.ad.m.prev| prevalence at the start of the simulation.
| | population size | | n0 | |
| | # of years | | n.years | # of years to run the simulation |
